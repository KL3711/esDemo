...
public class ImpData {
	...
	public void impData(){
		...
		dateType.put("index", "not_analyzed");
		JSONObject doubleType = new JSONObject();
		doubleType.put("type", "double");
		doubleType.put("index", "not_analyzed");
		...
		try {
			...
			/*************createStructure*************/
			//client.admin().indices().prepareDelete("dzdz_fpxx_zzsfp").get();
			//一会儿在7要素(代码，号码，开票日期，金额，税额，购销方识别号）上，添加一个mapping， "index":"not_analyzed"
...
//			System.out.println(zzsfpJson);
			/*************createStructure*************/
			//TransportClient client = ElasticConn.client();
			//CreateIndexResponse cir = client.admin().indices().prepareCreate("dzdz_fpxx_zzsfp").addMapping("fulltext", zzsfpJson.toString()).get();
			
			//CreateIndexResponse cir = client.admin().indices().prepareCreate("dzdz_fpxx_zzsfp").setSettings(settings).addMapping("fulltext", zzsfpJson.toString()).get();
//			
...
//			}
			...
			/************createData**************/
			//构造数据
			String zzsfpsql = "select * from dzdz_fpxx_zzsfp  where fphm='01137502' and fpdm='1100143130'";
			String hwxxsql = "select * from dzdz_hwxx_zzsfp where fphm='01137502' and fpdm='1100143130'";
...
//				ps.setString(2, fphm);
			...
			for (int i = 0; i <= 500000; i++) {
				...
				brb.add(client.prepareIndex("dzdz_fpxx_zzsfp", "fulltext").setSource(zzsfp));
				// IndexResponse ir = client.prepareIndex("dzdz",
				// "dzdz_fpxx_zzsfp").setSource(zzsfp).get();
				// System.out.println(ir.getId());
				if (i % 1000 == 0) {
					BulkResponse br = brb.get();
					if (br.hasFailures()) {
						System.out.println("below " + i + " has failure!");
